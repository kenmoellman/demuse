...
dbref door;
{
  /* dbref room; dbref thing;  for ( room = 0; room < db_top; room++ ) if (
     Typeof(room) == TYPE_ROOM )  { thing = Exits(room); while ( thing !=
     NOTHING )  { if ( thing == door ) return room; thing = db[thing].next; }
     }  return NOTHING; */
...
dbref remove_first_fighting(first, what)
dbref first;
dbref what;
{
  dbref prev;
  ...
  else
  {
    ...
    DOLIST(prev, first)
    {
      ...
      {
	db[prev].next_fighting = db[what].next_fighting;
	return first;
      }
    }
    return first;
...
static void get_thing_tz(thing, tzp, lyp)
dbref thing;
char **tzp;
int *lyp;
{
