...
#define LEFT_DELIMITER(x) ((x == NOT_TOKEN) || (x == '(') || (x == AT_TOKEN) || (x == IS_TOKEN) || (x == CARRY_TOKEN))
...
static dbref match_dbref(player, name)
dbref player;
char *name;
{
...
char *arg;
{
  ...
  while (*s)
  {
	...
	notify(player, tprintf("Warning: no such attribute '%s' on #%ld",
			       t, thing));
      t += strlen(t);
      if (*s != ':')
      {
...
static void eval_fun(buffer, str, doer, privs)
char *buffer;
char *str;
dbref doer;
dbref privs;
...
static int eval_boolexp1(object, key, zone)
dbref object;
char *key;
dbref zone;
{
...
static int test_atr(buf, player, ind)
char **buf;
dbref player;
int ind;
{
...
static int eval_boolexp_OR(buf)
char **buf;
{
  int left;
...
static int eval_boolexp_AND(buf)
char **buf;
{
  int left;
...
static int eval_boolexp_REF(buf)
char **buf;
{
  dbref thing;
  int type, t;
